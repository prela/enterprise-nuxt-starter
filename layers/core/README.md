...
| `NUXT_DATABASE_URL` | PostgreSQL URL (`postgres://` or `postgresql://`) mapped to Core `runtimeConfig.databaseUrl`. Missing, non-URL, or non-PostgreSQL values fail boot with a Zod error. Compose also sets `DATABASE_URL` to the same value for Drizzle / Identity later; that name is not a Core runtimeConfig key. |
